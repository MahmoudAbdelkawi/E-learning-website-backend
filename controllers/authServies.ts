...
const signup = expressAsyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const { email, password, fullName, role }: userType = req.body;
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (user) {
      ...
      if (passwordIsTure) {
        ...
        next(
          new ApiError("Error in Email Or Password", StatusCodes.UNAUTHORIZED)
        );
      }
    } else
      next(
...
const forgetPassword = expressAsyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (user) {
      ...
      user.verificationCode = sign(
        { code: num },
        `${process.env.JWT_SECRET_KEY_FOR_VERIFICATION_CODE}`,
        { expiresIn: `${process.env.EXPIRATION_TIME_FOR_VERIFICATION_CODE}` }
      );
      await user.save();
...
const confirmationCode = expressAsyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
...
const canChangePassword = expressAsyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const { email } = req.body;
    const user: any = await Users.findOne({ email });
